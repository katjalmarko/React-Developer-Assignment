  ...
  return (
    <div>
      ...
      {filteredToDoList.map((todo) => (
        <div key={todo.id} className={`absolute top-1/2 left-1/2 h-96 w-80 -translate-x-1/2 -translate-y-1/2 rotate-3 transform space-y-6 rounded-2xl bg-gray-100 p-6 duration-300 hover:rotate-0 ${todo.completion ? 'bg-green-400/50' : ''}`}>
          <section
          className="absolute top-1/2 left-1/2 h-96 w-80 -translate-x-1/2 -translate-y-1/2 rotate-3 transform space-y-6 rounded-2xl bg-gray-100 p-6 duration-300 hover:rotate-0"
        ></section>
          <div className="flex justify-end">
