...
const addTask = async () => {
  ...
  {filteredToDoList.map((todo) => (
    <div key={todo.id} className={`bg-gray-200 opacity-20 border rounded-md p-4 ${todo.completion ? 'bg-green-400 opacity-50' : ''}`}>
      <div className="flex w-full items-center bg-gray-900">
        <section
          className="absolute top-1/2 left-1/2 h-96 w-80 -translate-x-1/2 -translate-y-1/2 transform rounded-2xl bg-gray-100 p-6 duration-300 hover:rotate-0"
        >
          <div className="flex justify-end">
            <div className="h-4 w-4 rounded-full bg-gray-900"></div>
          </div>

          <header className="text-center text-xl font-extrabold text-gray-600 justify-center">
            Finish until:{" "}
