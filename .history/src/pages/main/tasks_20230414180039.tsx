...
const addTask = async () => {
  ...
  {filteredToDoList.map((todo) => (
    <div
      key={todo.id}
      className={`bg-gray-200/20 border rounded-md p-4 transition-all duration-500 ease-in-out ${
    ...
    >
      ...
      <div className="flex justify-between">
        <button
          ...
          className="bg-red-500/80 hover:bg-red-700/80 text-white font-bold py-2 px-4 rounded mt-2 transition-all duration-300 ease-in-out"
        >
          Delete Task
        </button>
        <button
          className={`bg-orange-500/80 hover:bg-orange-700/80 text-white font-bold py-2 px-4 rounded mt-2 transition-all duration-300 ease-in-out
          ${todo.completion ? 'bg-green-600/80 hover:bg-green-800/80' : ''}`}
          onClick={() => {
            if (todo.completion) {
