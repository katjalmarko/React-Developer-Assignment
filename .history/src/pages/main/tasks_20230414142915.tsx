...
const addTask = async () => {
  ...
  return (
    <div className="flex flex-col items-center m-12">
      ...
      <div className={`grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 w-full ${todo.completion ? 'bg-orange-200' : ''}`}>
  {toDoList.map((todo) => (
    <div key={todo.id} className="bg-gray-200/20 border rounded-md p-4">
      ...
      <div className='flex justify-between'>
        ...
        <button
          className={`bg-green-500 hover:bg-green-700 text-white font-bold py-2 px-4 rounded mt-2 ${todo.completion ? 'bg-orange-200' : ''}`}
          onClick={() => {
            if (todo.completion) {
