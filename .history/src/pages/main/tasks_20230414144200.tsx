        ...
        {toDoList.map((todo) => (
          <div key={todo.id} className={`bg-gray-200/20 border rounded-md p-4 ${todo.completion ? 'bg-green-400/50' : ''}`}>
            ...
            <div className='flex justify-between'>
              ...
              <button
                className={`bg-green-500/80 hover:bg-green-700/80 text-white font-bold py-2 px-4 rounded mt-2 
                              ${todo.completion ? 'bg-green-500/80 hover:bg-green-700/80bg-orange-500/80 hover:bg-orange-700/80' : ''}`}
                onClick={() => {
                  if (todo.completion) {
                  ...
                  >
                {todo.completion ? 'Done Task' : 'Completed'}
              </button>
            </div>
