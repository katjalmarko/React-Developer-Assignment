...
const getToDoList = async () => {
  ...
  const completeTask = async (id: string) => {
    try {
      ...
      await updateDoc(taskDoc, { completion: !isCompleted });
      const updatedToDoList = toDoList.map((task) =>
        task.id === id ? { ...task, completion: !isCompleted } : task
      );
  ...
  const taskCompletedStyle = "bg-green-300";
  const taskNotCompletedStyle = "bg-white";
