      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 w-full">
        {filteredToDoList.map((todo) => (
          <div key={todo.id} className={`bg-gray-200/20 h-60 w-80 border rounded-md p-4 rotate-3 transform duration-300 hover:rotate-0 ${todo.completion ? 'bg-green-400/50' : ''}`}>
              ...
              <h1 className="mb-2 text-center text-5xl font-bold mt-8 text-gray-900">{todo.title}</h1>
              <p className="flex justify-c mb-2 mt-8 text-center">Description: {todo.description}</p>
