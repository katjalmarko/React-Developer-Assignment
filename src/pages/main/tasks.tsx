...
function Tasks() {
  ...
  const handleAddTask = async (newTask: { title: string; description: string; date: Date }) => {
    try {
  ...
  const filteredTasks = toDoList
    ...
    .filter((task) => task.title.toLowerCase().includes(filterValue.toLowerCase()));
  ...
  return (
    <div className='flex flex-col items-center justify-center m-12 mb-80'>
      ...
      <h1
        className='text-4xl lg:text-5xl font-bold mt-12 mb-10 text-white tracking-widest'
        style={{ textShadow: '0 0 20px black' }}
      >
        Your To-Do List
      </h1>
      ...
      <div className='flex flex-wrap gap-6 justify-center'>
        {filteredTasks.map((task) => (
          ...
          >
            ...
            <h3
              className='text-center text-3xl font-bold mt-4 text-gray-900'
              style={{ textShadow: '0 0 10px white' }}
            >
              {task.title}
            </h3>
            <div
              className='flex justify-center text-center h-24'
